...
parser.add_argument("-bt", type=int, default=32)
...
for i, (train_label, dev_label) in enumerate(zip(train_labels, dev_labels)):
    ...
    dataloader_train = DataLoader(
        train_dataset, batch_size=args.bt, shuffle=True)
...
print("QWK=", cohen_kappa_score(dev_gold, dev_preds, weights='quadratic'))
...
with open(f"logs/{nowtime}.txt", "w") as f:
    ...
    f.write(f"\tbatch: {args.bt}\n")
    f.write(
        f"QWK={cohen_kappa_score(dev_gold, dev_preds, weights='quadratic')}\n")
