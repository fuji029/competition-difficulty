...
modelpaths = [args.path1, args.path2]
...
for i, (train_label, dev_label, modelpath) in enumerate(zip(train_labels, dev_labels, modelpaths)):
    ...
    model = Bert4Classification.load_from_checkpoint(
        modelpath, num_labels=6, lr=1e-6)
    with torch.no_grad():
        preds = list()
        logits = list()
